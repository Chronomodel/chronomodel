...
QVector<float> MetropolisVariable::runRawTraceForChain(const QList<ChainSpecs> &chains, const int index)
{
    ...
    if (mRawTrace.empty()) {
        ...
        for (int i=0; i<chains.size(); ++i) {
            ...
            if (i == index) {
               // std::copy(mRawTrace.begin()+shift+ burnAdaptSize, mRawTrace.begin()+shift+traceSize, trace.begin());
...
void MetropolisVariable::loadFromStreamOfQByteArray(QDataStream *in)
{
   ...
   /* int it = 0;
    ...
    std::generate(mRawTrace.begin(),mRawTrace.end(),[in]{QByteArray tmp; *in >> tmp; return tmp.toFloat();});
